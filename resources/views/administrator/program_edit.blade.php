...
@section('title_content', 'Ubah Data Program')

@section('content')
    <form action="{{ route('mutate_admin_program_edit', ['id' => $program->id]) }}" method="POST"
        ...
        <div class="mb-3">
            ...
            <img id="imagePreview" src="{{ uploads('storage/program/' . $program->image) }}" alt="Image Preview"
        ...
        <div class="mb-3">
            ...
            <img id="imagePreview2" src="{{ uploads('storage/program/' . $program->thumbnail) }}" alt="Image Preview"
                style="display: block; height:120px; padding-bottom:5px;">
        ...
        <div class="mb-3">
            ...
            <select class="form-select" id="dropwown-kategori" aria-label="Floating label disabled select example"
                name="id_categori" required>
                <option selected value="{{ $program->program_categories_id }}">Daftar Program</option>
                @foreach ($programCategorisList as $item)
                    <option value="{{ $item->id }}"
                        {{ $item->id == $program->program_categories_id ? 'selected' : '' }}>{{ $item->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" aria-label="Sizing example input"
                aria-describedby="inputGroup-sizing-default" required name="harga" value="{{ $program->harga }}"
                @error('harga') is-invalid @enderror value="{{ old('harga') }}">
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" aria-label="Sizing example input"
                ...
                @error('call_center') is-invalid @enderror value="{{ $program->call_center }}">
        </div>
        ...
        @if (Auth::user()->role == '')
            ...
            <div class="form-floating">
                <select class="form-select" id="dropwown-instansi" aria-label="Floating label disabled select example"
                    name="instansi">
                    <option selected value="{{ $program->instansi_id }}">Daftar Instansi</option>
                    @foreach ($instansiList as $item)
                        <option value="{{ $item->id }}" {{ $item->id == $program->instansi_id ? 'selected' : '' }}>
                            {{ $item->name }}
                        </option>
                    @endforeach
                </select>
